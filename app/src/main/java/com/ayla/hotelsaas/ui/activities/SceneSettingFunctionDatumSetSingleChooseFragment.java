    ...
    protected void initView(View view) {
        ...
        mRecyclerView.addItemDecoration(new RecyclerView.ItemDecoration() {
            ...
            public void getItemOffsets(@NonNull Rect outRect, @NonNull View view, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
                ...
                int sizeOne = SizeUtils.dp2px(60);
                int sizethree = SizeUtils.dp2px(45);
                int sizeTwo = SizeUtils.dp2px(30);
                final int itemPosition = parent.getChildAdapterPosition(view);
                final int itemCount = state.getItemCount();
                if ((itemCount == 1 && itemPosition == itemCount - 1) || (itemCount == 2 && itemPosition == itemCount - 1)) {
                    outRect.bottom = sizeOne;
                } else if (itemCount == 3 && itemPosition == itemCount - 1) {
                    outRect.bottom = sizethree;
                } else if (itemCount > 2 && itemPosition == itemCount - 1) {
                    outRect.bottom = sizeTwo;
                }
            }
        });
        mRecyclerView.setAdapter(mAdapter);

        attributesBean = (DeviceTemplateBean.AttributesBean)

                getArguments().

                        getSerializable("attributeBean");
