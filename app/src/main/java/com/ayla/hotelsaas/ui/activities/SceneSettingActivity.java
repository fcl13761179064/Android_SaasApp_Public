...
//                            .putExtra("position", position), REQUEST_CODE_EDIT_DELAY_ACTION);
    ...
    protected void appBarRightTvClicked() {
        if (null == mRuleEngineBean.getRuleName() || mRuleEngineBean.getRuleName().length() == 0 || mRuleEngineBean.getRuleName().trim().isEmpty()) {
            CustomToast.makeText(SceneSettingActivity.this, "名称不能为空", R.drawable.ic_toast_warning);
            return;
        }
        if (mRuleEngineBean.getConditions().size() == 0) {
            CustomToast.makeText(SceneSettingActivity.this, "请添加条件", R.drawable.ic_toast_warning);
            return;
        }
        if (mRuleEngineBean.getActions().size() == 0) {
            CustomToast.makeText(SceneSettingActivity.this, "请添加动作", R.drawable.ic_toast_warning);
            return;
        }
            ...
            for (BaseSceneBean.Condition condition : mRuleEngineBean.getConditions()) {
                if (condition instanceof BaseSceneBean.DeviceCondition) {
                    ...
                    if (exist.contains(value)) {
                        CustomToast.makeText(getBaseContext(), "选择满足所有条件时，条件中不可以添加多个同一设备的同一功能", R.drawable.ic_toast_warning);
                        return;
                    } else {
                        exist.add(value);
                    }
        ...
        if (lastAction instanceof BaseSceneBean.DelayAction) {//如果最后一个Action是延时，不允许
            CustomToast.makeText(getBaseContext(), "延时后必须添加一个设备类型的动作", R.drawable.ic_toast_warning);
            return;
        }
        for (BaseSceneBean.Condition condition : mRuleEngineBean.getConditions()) {
            if (condition instanceof BaseSceneBean.DeviceCondition) {
                ...
                if (devicesBean == null) {
                    CustomToast.makeText(getBaseContext(), "如想激活此联动，请先删除已移除的设备", R.drawable.ic_toast_warning);
                    return;
                } else {
                    if (devicesBean.getBindType() == 1) {
                        CustomToast.makeText(getBaseContext(), "请先绑定待添加的设备", R.drawable.ic_toast_warning);
                        return;
                    }
                }
            }
        ...
        for (BaseSceneBean.Action action : mRuleEngineBean.getActions()) {
            if (mRuleEngineBean.getConditions().size() == 1 && TextUtils.isEmpty(mRuleEngineBean.getConditions().get(0).getFunctionName())) {
                if (ThreeStringEques.mIsEques(action)) {
                    CustomToast.makeText(getBaseContext(), "一键执行不能同时作为条件和动作", R.drawable.ic_toast_warning);
                    return;
                }
            }
