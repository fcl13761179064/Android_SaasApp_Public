...
public class ForgitPassWordActivity extends BaseMvpActivity<ForgitView, ForgitPresenter> implements ForgitView {
        ...
        tv_send_code.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (TextUtils.isEmpty(userName)) {
                    CustomToast.makeText(MyApplication.getContext(), "手机号不能为空", R.drawable.ic_toast_warning);
                    return;
                } else if (!RegexUtils.isMobileSimple(userName)) {
                    CustomToast.makeText(MyApplication.getContext(), "手机格式错误", R.drawable.ic_toast_warning);
                    return;
                }
    ...
    private void modifyPassword() {
        ...
        if (TextUtils.isEmpty(yanzhengma)) {
            CustomToast.makeText(MyApplication.getContext(), "验证码不能为空", R.drawable.ic_toast_warning);
            errorShake(2, 2, "");
            return;
        }
    ...
    public void resertPasswordFailed(Throwable throwable) {
        CustomToast.makeText(this, TempUtils.getLocalErrorMsg("重置密码失败", throwable), R.drawable.ic_toast_warning);
    }
    ...
    public void resetPassword(String phone) {
        ...
        if (TextUtils.isEmpty(new_password)) {
            CustomToast.makeText(MyApplication.getContext(), "密码不能为空", R.drawable.ic_toast_warning);
            return;
        } else if (new_password.length() < 6) {
            CustomToast.makeText(MyApplication.getContext(), "密码至少为6位", R.drawable.ic_toast_warning);
            return;
        }
