...
public class SwitchUsageSettingActivity extends BaseMvpActivity<SwitchUsageSettingView, SwitchUsageSettingPresenter> implements SwitchUsageSettingView {
            ...
            mBinding.button2.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (TextUtils.isEmpty(name)) {
                        CustomToast.makeText(getApplicationContext(), "名称不能为空", R.drawable.ic_toast_warning);
                        return;
                    }
                    for (int i = 0; i < selfNames.length; i++) {
                        if (i < position - 1) {
                            if (TextUtils.equals(name, selfNames[i])) {
                                CustomToast.makeText(getApplicationContext(), "名称重复，请修改", R.drawable.ic_toast_warning);
                                return;
                            }
                        }
                    }
    ...
    public void saveFailed(Throwable throwable) {
        CustomToast.makeText(getApplicationContext(), "保存失败", R.drawable.ic_toast_warning);
    }

    @Override
    public void renameFail(String throwable) {
        CustomToast.makeText(getApplicationContext(), throwable, R.drawable.ic_toast_warning);
    }

    @Override
    public void saveSuccess() {
        CustomToast.makeText(getApplicationContext(), "保存成功", R.drawable.ic_toast_warning);
        setResult(RESULT_OK);
        finish();
    }
}
