...
public class SceneSettingFunctionSelectPresenter extends BasePresenter<SceneSettingFunctionSelectView> {
     ...
     * @param deviceId
     */
    public void loadAllProperty(boolean condition, String deviceId, String pid) {
        Disposable subscribe = RequestModel.getInstance()
                    ...
                    public List<DeviceTemplateBean.AttributesBean> apply(@NonNull List<String> properties, @NonNull DeviceTemplateBean deviceTemplateBean) throws Exception {
                        ...
                        if (TempUtils.isINFRARED_VIRTUAL_SUB_DEVICE(devicesBean) && !condition) {//如果是用途设备(红外遥控家电)，就直接套用物模型作为联动动作，不走品类中心过滤
                            ...
                            for (String property : properties) {
                                if (!property.equals(ConstantValue.SCENE_TEMPLATE_CODE) && property.contains(".") && (deviceTemplateBean != null && deviceTemplateBean.getEvents() != null)) {//event事件类型
                ...
                .subscribe(new Consumer<List<DeviceTemplateBean.AttributesBean>>() {
                    ...
                    public void accept(List<DeviceTemplateBean.AttributesBean> attributesBeans) throws Exception {
                        mView.showAllProperty(attributesBeans);
                    }
                }, new Consumer<Throwable>() {
                    @Override
                    public void accept(Throwable throwable) throws Exception {
                        mView.showAllProperty(null);
                    }
                });
        addSubscrebe(subscribe);
    }

    public void getGroupAbility(String groupId) {
        if (mView != null) {
            mView.showProgress("加载中");
        }
        Disposable subscribe = RequestModel.getInstance().getGroupAbility(groupId).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(groupAbilities -> {
            if (mView != null) {
                mView.loadGroupAbilitySuccess(groupAbilities);
            }
            mView.hideProgress();
        }, throwable -> {
            if (mView != null) {
                mView.loadGroupAbilityFail(throwable);
            }
            mView.hideProgress();
        });
        addSubscrebe(subscribe);
