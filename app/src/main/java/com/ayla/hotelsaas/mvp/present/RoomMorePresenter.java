...
public class RoomMorePresenter extends BasePresenter<RoomMoreView> {
    ...
    public void A6CompleteTransferToZj(long mScopeId, int billId) {
        Disposable subscribe = RequestModel.getInstance().CompletetransferUpdateRoomToZj( mScopeId,billId)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(new Consumer<Disposable>() {
                    @Override
                ...
                .subscribe(new Consumer<BaseResult>() {
                    ...
                    public void accept(BaseResult result) throws Exception {
                        mView.ComPletetransferSuccess(result);
                    }
                }, new Consumer<Throwable>() {
                    @Override
                    public void accept(Throwable throwable) throws Exception {
                        mView.CompletetransferFailed(throwable);
                    }
                });
        addSubscrebe(subscribe);
    }
