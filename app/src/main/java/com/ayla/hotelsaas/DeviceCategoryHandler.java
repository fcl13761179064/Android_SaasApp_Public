...
import com.ayla.hotelsaas.ui.activities.A2GatewayAddGuideActivity;
import com.ayla.hotelsaas.ui.activities.A2GatewaySelectActivity;
import com.ayla.hotelsaas.ui.activities.AylaGatewayAddGuideActivity;
import com.ayla.hotelsaas.utils.CustomToast;
import com.ayla.hotelsaas.ui.activities.DeviceAddGuideActivity;
import com.ayla.hotelsaas.ui.activities.GatewaySelectActivity;
import com.ayla.hotelsaas.ui.activities.HongyanGatewayAddGuideActivity;
import com.ayla.hotelsaas.ui.activities.ble_ap.BleApDeviceGuideActivity;
...
//            handleShouldExit();
    ...
    public void handleAddJump(DeviceCategoryBean.SubBean.NodeBean[] subBeans) {
        ...
        List<DeviceListBean.DevicesBean> devices = MyApplication.getInstance().getDevicesBean().getDevices();
        ...
        if (subBeans.length == 1) {
            ...
            boolean is_has_A6 = loadA6Gateway(aylaGateways);
            if ((is_has_A6 || is_has_A2) && nodeBean.getOemModel().size() > 1) {//A2网关
                ...
                if (networkType == 6) {
                    Intent intent = new Intent(fromContext, BleApDeviceGuideActivity.class);
                    intent.putExtra(KEYS.PRODUCTBEAN, nodeBean);
                    intent.putExtra(KEYS.PID,nodeBean.getPid());
                ...
                if (networkType == 2) {//艾拉网关
                    ...
                    } else if (nodeBean.getPid().equalsIgnoreCase("ZBGW0-A000003")) {//这是A6网关引导页，和A2一样的页面
                        Intent mainActivity = new Intent(fromContext, A2GatewayAddGuideActivity.class);
                        mainActivity.putExtra("addInfo", addInfo);
                        startActivityForResult(mainActivity, REQUEST_CODE_ADD_DEVICE);
                    } else {
                    ...
                    if (aylaGateways.size() == 0) {//没有艾拉网关
                        CustomToast.makeText(fromContext, "请先绑定网关", R.drawable.ic_toast_warning);
                        ...
                        if (TempUtils.isDeviceOnline(gateway)) {//网关在线
                            ...
                            CustomToast.makeText(fromContext, "当前网关离线", R.drawable.ic_toast_warning);
                        }
                    } else {//多个网关
    ...
    //加载A6网关
    public boolean loadA6Gateway(List<DeviceListBean.DevicesBean> aylaGateways) {
        if (aylaGateways != null && aylaGateways.size() > 0) {
            for (DeviceListBean.DevicesBean device : aylaGateways) {
                if (TempUtils.isDeviceGateway(device) && device.getPid().equalsIgnoreCase("ZBGW0-A000003")) {
                    return true;
                }
            }
        }
    ...
    private int calculateNetworkType(DeviceCategoryBean.SubBean.NodeBean subBean) {
        int networkType = -1;//2：艾拉网关  3：艾拉节点 5：艾拉WiFi   1：鸿雁网关 4：鸿雁节点 6 wifi-ble 设备
        if (subBean.getSource() == 0) {//艾拉云
            if (subBean.getProductType() == 1) {//网关设备
                ...
                if (subBean.getIsNeedGateway() == 1) {//节点设备
                    networkType = 3;//艾拉节点
                } else if ("2,3".equals(subBean.getWifiNetworkMode())) {
                    //蓝牙和ap配网
                    networkType = 6;
                } else {
